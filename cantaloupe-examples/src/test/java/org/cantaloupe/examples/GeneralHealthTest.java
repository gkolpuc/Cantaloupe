...
public class GeneralHealthTest {
	...
	public void shouldReturnTotalCount() throws UnirestException, IOException {
		Assert.assertTrue(TestStatus.Success == Cantaloupe.json(API.search().body(Resource.epReq("minimumRequest.json"))
				.asJson()).getLongVal("totalCount").assertNotNull().status());
	}
