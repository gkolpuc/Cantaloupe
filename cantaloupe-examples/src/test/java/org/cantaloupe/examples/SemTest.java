...
public class SemTest {
	...
	public void shouldReturnZeroEvents() throws UnirestException, IOException {
		Assert.assertTrue(TestStatus.Success == Cantaloupe.json(ES.search()
				.body(esReq("AdditionalMaterialsWithDuplicatedLink.json")).asJson()).getArray("hits.hits")
				.count(equalTo(ZERO)).status());
	}
}
