...
public class HqChangeTest {
	...
	public void shouldReturnHqEvents() throws UnirestException, IOException {
		Assert.assertTrue(TestStatus.Success == Cantaloupe.json(API.search().body(Resource.epReq("hq_change_all.json"))
				.asJson()).getArray("events").forEach(exists("EventData.CHQCEvent")).status());
	}
}
